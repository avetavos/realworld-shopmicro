...
ตอนนี้มีไฟล์ `.proto` จริงแล้ว มา resolve dependency `googleapis` ที่ `buf.yaml` ของ [Protobuf Tooling →](/shopmicro/th/setup/proto-tooling/) ประกาศไว้กัน — ขั้นตอนนี้เป็นไปไม่ได้ตอนที่ `proto/` ยังว่างเปล่า และมันจะเขียน `buf.lock` เพื่อ pin commit ของ `googleapis` ให้ทุกคนตรงกัน:
