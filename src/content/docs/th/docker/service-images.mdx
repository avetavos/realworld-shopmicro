...
runtime base คือ `gcr.io/distroless/static-debian13:nonroot` — tag ที่ระบุเวอร์ชัน Debian ชัดเจน ไม่ใช่ alias เปล่า ๆ `static:nonroot` ทั้งสอง tag ชี้ไป image เดียวกันวันนี้ แต่ทีม distroless เตือนไว้ชัดเจนว่า tag ที่ไม่ระบุเวอร์ชันจะย้ายไป Debian รุ่นใหม่กว่าในอนาคตโดยไม่แจ้งล่วงหน้า การ pin `-debian13` คือสิ่งที่ป้องกันไม่ให้การ rebuild เปลี่ยน base image ไปเงียบ ๆ โดยที่คุณไม่รู้ตัว คำว่า "Distroless" หมายถึงไม่มี shell ไม่มี package manager ไม่มี coreutils มีแค่ไฟล์ไม่กี่ตัวที่ static binary ต้องใช้ ได้แก่ CA certificate, timezone data และ `/etc/passwd`
