...
**Migration service แบบ one-shot** SQL migration [Repo Layout →](/shopmicro/th/setup/repo-layout/) ไม่ถูก bake เข้า image ของเซอร์วิส เพราะ `.dockerignore` ของ [Service images →](/shopmicro/th/docker/service-images/) กันออกอย่างตั้งใจ แต่จะรันเป็นเซอร์วิส Compose อายุสั้นของตัวเองด้วย image `migrate/migrate` ที่ apply migration ค้างทุกตัวกับ database แล้ว exit `0` จากนั้น app service ก็พึ่ง migration job ของตัวเองด้วย `condition: service_completed_successfully` Order จึงไม่มีทางเริ่มขึ้นมาพร้อม schema ที่ยังไม่ถูกสร้าง การแยก migration ออกจากเซอร์วิสที่ใช้ schema คือวินัยเดียวกับที่คอร์สนี้ใช้ทุกที่ — แยกสิ่งที่ *เปลี่ยน* database ออกจากสิ่งที่ *ใช้* database
...
`migrate-*` job สองตัวจะไม่โผล่ใน `ps` เพราะรันแล้ว exit `0` จบไปแล้ว ทีนี้ลองรัน end-to-end smoke test *ชุดเดิม* จากบทก่อน ๆ [The Saga Handler →](/shopmicro/th/saga/orchestration/) เพียงแต่ทุก hop ตอนนี้เกิดระหว่าง container และสิ่งเดียวที่คุณคุยด้วยคือ gateway บน `8080` ของ host:
