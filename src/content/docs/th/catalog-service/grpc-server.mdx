...
`pkg/pg/pg.go` เป็น import จริงตัวแรกของ `pgxpool` ในคอร์สนี้ และ `pgxpool` ดึง `github.com/jackc/puddle/v2` เข้ามาด้วย — dependency ทางอ้อมที่ `go get github.com/jackc/pgx/v5` ใน [Go Module & Dependencies →](/shopmicro/th/setup/go-workspace/) ไม่เคยบันทึก `go.sum` ไว้ให้ เพราะตอนนั้นยังไม่มีใคร import `pgxpool` เลย รัน `go mod tidy` ก่อน:
