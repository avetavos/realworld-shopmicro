...
ตาราง `products` ผ่าน migration ของ `golang-migrate` และ `services/catalog/internal/repo/products.go` — `ProductRepo` ที่มีสามเมธอด (`List`, `Get`, `Create`) คุยกับตารางนี้ด้วย SQL ดิบผ่าน `pgx` นี่คือชั้นคั่นระหว่างฐานข้อมูลกับส่วนอื่นทั้งหมด placeholder ของ [The gRPC Server →](/shopmicro/th/catalog-service/grpc-server/) ยังไม่ต้องใช้ชั้นนี้ แต่ [The Product API →](/shopmicro/th/catalog-service/product-api/) เรียกเข้ามาที่นี่โดยตรงสำหรับ RPC ทั้งสามตัว
