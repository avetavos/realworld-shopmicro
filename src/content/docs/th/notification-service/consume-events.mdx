...
[Exchanges & Queues →](/shopmicro/th/rabbitmq/exchanges-queues/) ตั้งชื่อการแบ่งนี้ไว้ชัดแล้ว: log ที่ replay ได้ของ Kafka ตอบคำถาม "ทุกฝ่ายที่สนใจเห็น fact นี้แล้วหรือยัง" ส่วน work queue ของ RabbitMQ ตอบ "ยังมี job ที่ต้องทำครั้งเดียวค้างอยู่ไหม" Notification คือรอยต่อที่ fact แปลงร่างเป็น job
...
การโยนงานให้ RabbitMQ แทนทำให้ `Handle` return ในระดับไมโครวินาที offset commit ทันที และทุกส่วนที่ช้าและพังได้ของการส่ง notification ย้ายไปอยู่ใน process แยก (worker) ที่อ่าน queue แยก ซึ่งกลไก ack/retry/dead-letter ของ RabbitMQ ที่ [Acks, Retry & Dead Letters →](/shopmicro/th/rabbitmq/acks-retry-dlq/) สร้างไว้คือเครื่องมือที่เหมาะกับงานนั้นพอดี

และ consumer group ของ Notification เป็นของตัวเอง: `NewConsumer(brokers, "notification", "payments")` saga ของ Order อ่าน topic `"payments"` เดียวกันภายใต้ group `"order"` [The Saga Handler →](/shopmicro/th/saga/orchestration/) ส่วน Notification อ่าน topic เดียวกันนั้นภายใต้ group `"notification"` กฎของ [Topics, Partitions & Consumer Groups →](/shopmicro/th/kafka/topics-groups/) คือสิ่งที่ทำให้ปลอดภัย: consumer ที่อยู่คนละ group จะได้อ่าน topic ทั้งหมดครบถ้วนและเป็นอิสระต่อกัน
...
call นั้น idempotent [Exchanges & Queues →](/shopmicro/th/rabbitmq/exchanges-queues/) การ declare queue `notification.send` พร้อม dead-letter setup ซ้ำจึงไม่ทำอะไรเลยถ้า worker หรือ run ก่อนหน้า declare ไว้แล้ว ปลอดภัยที่ทั้งเซอร์วิส Notification และ worker จะ declare topology เดียวกันแยกกัน ตัวไหนเริ่มก่อนก็ชนะไป ส่วนอีกตัวกลายเป็น no-op
...
ภายในไม่กี่วินาที — ช่วงที่ outbox relay publish `order.created`, Payment publish `payment.succeeded` แล้ว Notification consume ต่อ — จะมี send-job หนึ่งอันค้างอยู่บน `notification.send` ยืนยันได้ใน management UI ที่ [http://localhost:15672](http://localhost:15672) (login `shopmicro` / `shopmicro`) → **Queues and Streams** → `notification.send` โดย **Messages Ready** ต้องเป็น `1` คลิกที่ queue เปิด **Get messages** แล้วอ่าน body ได้เลย (ตั้ง **Requeue: Yes** ไว้ เพื่อไม่ให้ consume job ทิ้งไป):
...
job อันนี้เกิดจาก event ของ **Kafka** และตอนนี้รออยู่บน queue ของ **RabbitMQ** เท่ากับพิสูจน์ hop ข้ามสอง broker แล้ว สังเกตว่า `notification.send` มี message *ready* ค้างไว้โดยไม่มี consumer มาดึงออก ไม่มีอะไรหายระหว่างที่ worker ยัง down เพราะ durable queue เก็บ job ไว้จนกว่าจะมี worker มา ack ตรงตามการรับประกันที่ [Exchanges & Queues →](/shopmicro/th/rabbitmq/exchanges-queues/) วางไว้ ทีนี้ลองวางคำสั่งซื้อขนาดใหญ่เพื่อดูว่า message ฝั่งล้มเหลวก็เข้า queue เหมือนกัน:
...
`curl` ผ่าน gateway วาง job จริงสองอันบน `notification.send` มองเห็นได้และ durable ใน management UI ทั้งที่ยังไม่มี worker รันมา consume เลย บทถัดไป [The Send Worker →](/shopmicro/th/notification-service/worker-deliver/) คือ binary แยกที่ดึง queue นั้น "ส่ง" notification แต่ละอัน แล้ว ack ทิ้ง — เอากลไก ack/retry/dead-letter จาก [Acks, Retry & Dead Letters →](/shopmicro/th/rabbitmq/acks-retry-dlq/) มาใช้จริง
