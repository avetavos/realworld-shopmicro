...
เพื่อพิสูจน์ว่า wrapper นี้ใช้งานได้จริงกับ broker จริงก่อนจะมีใครมาพึ่งพา บทนี้ยังเขียนโปรแกรมเล็ก ๆ แบบใช้แล้วทิ้งสองตัว — `cmd/kafkademo/produce` กับ `cmd/kafkademo/consume` — ที่ publish event หนึ่งตัวแล้ว consume กลับมา กับ container Kafka ที่ [Infra & Compose →](/shopmicro/th/setup/infra-compose/) สร้างไว้แล้วใน Module 1
...
เซฟไฟล์นี้เป็น `pkg/kafka/kafka.go` [Go Module & Dependencies →](/shopmicro/th/setup/go-workspace/) รัน `go get github.com/segmentio/kafka-go` ไปแล้วตั้งแต่ Module 1 แต่ `go mod tidy` ของ [Code Generation →](/shopmicro/th/proto-grpc/codegen/) (ครั้งแรกที่คอร์สนี้รันคำสั่งนี้) ได้ตัด requirement นั้นออกจาก `go.mod` ไปแล้ว เพราะตอนนั้นยังไม่มีใคร import `kafka-go` เลย — เหตุผลเดียวกับที่ [The gRPC Server →](/shopmicro/th/catalog-service/grpc-server/) ต้องรัน `go mod tidy` อีกครั้งเพื่อ `pgxpool` โค้ด `pkg/kafka/kafka.go` ข้างบนคือ import จริงตัวแรก จึงต้องรันมันอีกครั้งตอนนี้:
...
ไม่มี output แปลว่าสำเร็จ (ถ้าข้าม `go mod tidy` ข้างบน จะ fail ด้วย `no required module provides package github.com/segmentio/kafka-go`)
