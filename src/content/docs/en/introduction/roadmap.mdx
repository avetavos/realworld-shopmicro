...
description: "ShopMicro is project #3 of a seven-project series that adapts the whole Learn Hub catalog into real applications. Here's where it fits and what's around it."
sidebar:
  order: 4
---
...
ShopMicro is **project #3** of a seven-project **Real-World Projects** series. Each project takes a different real application and rebuilds it from an empty repo, deliberately drawing on a different slice of the Learn Hub courses — so that, taken together, the seven cover almost the entire catalog. All seven are **live** today; ShopMicro is the one you're building now, and the four after it finish the map across mobile, micro-frontends, the offline web, and real cloud infrastructure.
...
A storefront composed of independently deployed micro-frontends: a React shell composes React, Svelte, and Astro remotes at runtime with Module Federation, Web Components are the universal boundary between frameworks, and each slice owns its UI and a thin BFF. You build a shared design system, cross-slice communication, host-owned routing, resilience when a remote is down, and per-slice deployment — where ShopMicro splits a backend into independent services, Mosaic splits a frontend into independent apps.
...
The capstone: take **ShopMicro** — this project's Helm chart — and **deploy it for real**. You provision cloud infrastructure as code across three clouds, run it on Kubernetes, and add authentication, feature flags, and observability.
...
All seven projects are live. This guide builds **ShopMicro** — head to the [Overview](/shopmicro/en/introduction/overview/) and start from an empty repo.
