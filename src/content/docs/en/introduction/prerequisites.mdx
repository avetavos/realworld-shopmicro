...
- [ ] **Go 1.25+** — compile and run all five services
...
Expected output (the version must be **1.25 or higher** — earlier Go doesn't have `sync.WaitGroup.Go`, which the saga module uses):
...
`kubectl` is the command-line client for Kubernetes; `helm` is the package manager we use to install ShopMicro's chart in Module 14 (this course uses **Helm 4**). Install both via Homebrew:
