...
**A freshness note on these three commands.** Since Broadcom's 2025 restructuring of Bitnami's catalog, the free tier of these OCI charts only ships an unpinned `:latest` image tag — the charts still install and run (verified above against a real `kind` cluster), but `helm install` now prints `WARNING: Rolling tag detected (bitnami/postgresql:latest), ...` for every Bitnami image the chart pulls, pointing at Broadcom's Bitnami Secure Images docs. That's expected noise, not a failure — `helm install`/`kubectl get pods` still succeed exactly as shown below — but it's worth knowing this is no longer a versioned, reproducible pull the way a pinned tag would be, and a production cluster would either pin a Bitnami Secure Images subscription tag or run its own Postgres/Kafka/RabbitMQ images instead.
